			...
			<div class="infoo">
				...
				<div class="inforight fr">
					...
					<div class="compile" v-show="isShowEdit">
						...
						<div class="compile-top">
							...
							<div class="input">
								...
								<p>
									...
									<input
										type="text"
										v-model="modelObj.SUBJECT_DEPT_ID_NAME"
										:disabled="isDisabledInput"><br></p>
								<div class="block"  style="display: inline-block;margin-left:0px;margin-bottom:10px ">
									...
									<select class="month" v-model="modelObj.FINANCE_CYCLE">
										<option :value="item.K" v-for="item in dateMonth">{{item.V}}</option>
									</select>
								</div>
								<p>
									<span>录入人：</span>
						...
						<div class="addmingxi">
							<table border="1" ref="mytable" style="width:80%;margin-top:30px;margin-left:80px;">
								<thead>
									<tr>
										<th>员工</th>
										<th>事由</th>
                              ...
                              <span>职务</span>
										...
										<td>
											<select name="" v-model="item.REASON_TYPE_ID_NAME" >
												<option :value="typeItem.ID" v-for="typeItem in detailTypeList.rows">{{typeItem.NAME}}</option>
											</select>
										</td>
										<td>
											<input type="text" v-model="item.BEGIN_DATE" :disabled="isDisabledInput">
											<input type="text" v-model="item.DEPT_ID" disabled v-show="false">
										</td>


										<td>
											<input type="text" v-model="item.END_DATE" >
										</td>
										<td>
											<input
												type="text"
...
//					     return time.getTime() < Date.now() - 8.64e7;
		...
		methods: {
			...
			cmdSave() {
				...
				var url = this.common.baseNodeUrl+"hrm/org/Srv_HRM_AttendanceSave.ashx";
			...
			cmdSubmit() {
				...
				this.$confirm('您确定要提交', '是否继续?', '提示', {
					...
					var strDataJson = JSON5.stringify(this.modelObj);
					var url = this.common.baseNodeUrl+"hrm/org/Srv_HRM_AttendanceSubmit.ashx";
					this.$http.post(url, strDataJson, {
						emulateJSON: true
					}).then(function(res) {
						var dataJson = res.body;
			...
			cmdAudit() {
				this.$confirm('您确定要审核', '是否继续?', '提示', {
					...
					var url = this.common.baseNodeUrl+ "hrm/org/Srv_HRM_AttendanceAudit.ashx";
			...
			cmdLinkRemove(item, index) {
				...
				var url = this.common.baseNodeUrl+ "hrm/org/Srv_HRM_AttendanceDel.ashx";
				var ids = "['"+item.ID+"']";
			...
			cmdDetailLinkRemove(item, index) {
				...
				if(item.ID != undefined && item.ID != "") {
					var ids = "['"+item.ID+"']";
					var url = this.common.baseNodeUrl+"hrm/org/Srv_HRM_AttendanceDetailDel.ashx";
					this.$http.post(url,ids, {
							emulateJSON: true
						})
						.then(function(res) {
		...
		created() {
			this.getMonths();
			// console.log('ff11ff');
			// console.log(add_months((Date.now(),-2), 'YY/YY/MM'))
