        ...
        <ul class="fr clearfix">
            <li class="fl " id="vacation">
                ...
                <select style="border: none" >
                    <option value="2017" selected>1个月内</option>
      ...
      this.$http.get(url)                
					.then(function(res) {
						...
						if(dataJson.Succeed != 1) {
							...
							console.log("证书到期提醒数据失败");
							return false;
						}
						this.tableResultList = dataJson.Message;
					}).catch(function (res) {
	        			  console.log(res+"证书到期提醒数据异常")
					});		
    },
    // 导出
    cmdDetailExpExcel(){
        var url = this.common.baseNodeUrl+"hrm/rpt/Srv_HRM_CertificateExpireExpExcel.ashx";
         this.$http.get(url)             
					.then(function(res) {
						var dataJson = res.body;
						if(dataJson.Succeed != 1) {
