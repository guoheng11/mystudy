<template>
    <div>
        <input type="text" class="searchBoxDept" v-model="boxChangeValue" @input="searchDataChange">
        <transition name="fold">
            <div class="showTable" v-show="isBox" @click.stop="pageBoxClick" style="position:absolute ;z-index:9999;">
                <!-- style="left:200px" -->
                <div class="tablehead">
                    <ul>
                        <!-- style="width:200px" -->
...
export default {
    ...
    methods: {
        ...
        //输入改变值
        searchDataChange() {
            if (!this.boxChangeValue.replace(/(^\s*)|(\s*$)/g, '')) {
                this.isBox = false;
                return;
            }
            this.boxIndex = 1;
    ...
    created(){
        this.boxChangeValue = this.inputValue;
    },
}
</script>
