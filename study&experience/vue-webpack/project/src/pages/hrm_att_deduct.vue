<template>
	<div @click="searchBoxListHidden">
		<div class="content">
			<div class="infoPlace ">
				您所在的位置是：
				<span>人事管理 / 考勤扣款登记</span>
			</div>
			<div class="infoo">
				<div class="infolinfoleft fl">
					<div class="infoTitle ">
						<div class="roster">
							<span>|</span>&nbsp;考勤扣款登记</div>
						<div>
							<ul class="clearfix">
				...
				<div class="inforight fr">
					...
					<div class="compile" v-show="isShowEdit">
							...
							<div class="input">
								...
								<div class="block"  style="display: inline-block;margin-left:0px;margin-bottom:10px ">
									...
									<select class="month" v-model="modelObj.FINANCE_CYCLE">
										<option :value="modelObj.FINANCE_CYCLE" v-for="value in dateMonth" >{{value}}</option>
									</select>
								</div>
								<p>
									<span>录入人：</span>
								...
								<p>
									<span>金额合计：</span>
									<input type="text" v-model="modelObj.TOTAL" :disabled="isDisabledInput"><br></p>
								<p>
									<span>备注：</span>
									<input
										type="text"
										v-model="modelObj.SUMARRY"
										style="height: 99px;"
										:disabled="isDisabledInput"></p>
							</div>
						</div>
						<ul class="clearfix compile-bottom"
							style="padding-left: 150px;padding-right: 20px;margin-top: 10px">
							<li class="fl">
							...
							<li class="fl">
								<el-button type="text" :disabled="btnActive.isDetailAdd" @click="">
									<e class="kaoqin"></e>从考勤生成</el-button>
							</li>
							<li class="fl">
								<el-button type="text" :disabled="btnActive.isDetailAdd" @click="">
						...
						<div class="addmingxi" style="padding-left:100px">
							<table border="1" ref="mytable" style="width:720px;margin-top:30px;margin-left:80px;">
								...
								<tbody v-for="(item ,index) in modelObj.DetailList " id="tbodyinput">
									<tr>
										<td>
											<input type="text"
												v-model="item.STAFF_ID_NAME"
												:disabled="isDisabledInput"
												@click="searchDetailStaffDataChange(index,$event);"
												@input="searchDetailStaffDataChange(index,$event);">
											<input type="text" v-model="item.STAFF_ID" v-show="false">
											<div v-show="isDetailBoxStaff"
												 @click.stop="pageDetailStaffClick"
										...
										<div
											style=""
											class="searchListDiv example">
											<table border="1" align="center"

												class="searchList">
												<tbody>
													<tr @click="selectDetailStaff(item);"
														@mouseover="boxMouseOver(index)"
														@mouseout="boxMouseOut(index)"
														:class="{'trlistover': boxMouseOverbgc===index}"
														v-for="(item ,index) in searchDetailStaffDataList.rows">
										...
										<div class="tablefoot">
											<div class="block pageDetailStaffClick"
														@click.stop="pageDetailStaffClick">
														<el-pagination
															@size-change="handleSizeChangeDetailStaff"
															@current-change="handleCurrentChangeDetailStaff"
															:current-page.sync="currentDetailPageStaff"
															:page-size="currentDetailPageSizeStaff"
															layout="total,prev, next, jumper"
															:total="searchDetailStaffDataList.total">
														</el-pagination>
											</div>
										</div>
									</div>
										...
										<td>
											<select style="width:70px;height:30px;" name="" v-model="item.REASON_TYPE_ID_NAME" >
...
<script>
    import {format,addMonths, compareAsc} from 'date-fns'
...
// 参数：{"ID":"主新增=空,修改=主键","CODE":"编码","NAME":"名称","FINANCE_CYCLE":"考勤月份","CREATE_DATE":"记录日期",
// "USER_ID":"记录人ID","USER_ID_NAME":"记录人名称","DEPT_ID":"记录部门ID","DEPT_ID_NAME":"记录部门名称","COMMENTS":"备注","STATE":1,"STATE_NAME":"1编辑","TOTAL":"总金额","DetailList":[{"ID":"主键","BASE_ID":"父主键","DEPT_ID":"部门(科室)ID","DEPT_ID_NAME":"部门(科室)名称","STAFF_ID":"员工ID","STAFF_ID_NAME":"员工姓名","REASON_TYPE_ID":"事由","REASON_TYPE_ID_NAME":"事由名称","MONEY":"金额","DAYS":"天数","COMMENTS":"备注"},{..},{..}]}}
				...
				detailTypeList:{total:0,rows:[]},//事由对象定义
			}
		},
		// filters: {
		// },
		methods: {
			...
			cmdSearchList() {
				...
				var url = this.common.baseNodeUrl+"hrm/atd/Srv_HRM_AttendDeductList.ashx" + param
				this.$http.get(url)
					.then(function(res) {
						var dataJson = res.body;
						if(dataJson.Succeed != 1) {
							...
							console.log("考勤扣款查询列表数据失败");
							return false;
						}
						this.tableResultList = dataJson.Message;
					}).catch(function (res) {
	        			  console.log(res+"考勤扣款查询列表数据异常")
					});
			...
			cmdSave() {
				...
				var url = this.common.baseNodeUrl+"hrm/atd/Srv_HRM_AttendDeductSave.ashx";
			...
			cmdSubmit() {
				...
				this.$confirm('您确定要提交', '是否继续?', '提示', {
					...
					var strDataJson = JSON5.stringify(this.modelObj);
					var url = this.common.baseNodeUrl+"hrm/atd/Srv_HRM_AttendDeductSubmit.ashx";
					this.$http.post(url, strDataJson, {
						emulateJSON: true
					}).then(function(res) {
						var dataJson = res.body;
			...
			saveIn() {
				...
				for(var k in this.modelObj.DetailList) {
					...
					// if(!this.modelObj.DetailList[k].TO_DEPT_ID_NAME) {
					// 	alert("请选择科室")
					// 	return false;
					// }

					if(!this.modelObj.DetailList[k].REASON_TYPE_ID_NAME) {
						alert("请选择事由")
						return false;
					}
			...
			cmdAudit() {
				this.$confirm('您确定要审核', '是否继续?', '提示', {
					...
					var url = this.common.baseNodeUrl+ "hrm/atd/Srv_HRM_AttendDeductAudit.ashx";
			...
			cmdLinkLook(item){
				...
				this.clearModelObj();
				var url = this.common.baseNodeUrl+ "hrm/atd/Srv_HRM_AttendDeductGet.ashx?ID=" + item.ID;
				this.$http.get(url)
					.then(function(res) {
						var dataJson = res.body;
						if(dataJson.Succeed != 1) {
			...
			cmdLinkEdit(item,act) {
				this.isShowEdit = true;
				this.clearModelObj();
				var url = this.common.baseNodeUrl+ "hrm/atd/Srv_HRM_AttendDeductGet.ashx.ashx?ID=" + item.ID;
				this.$http.get(url)
					.then(function(res) {
						var dataJson = res.body;
						if(dataJson.Succeed != 1) {
			...
			cmdLinkRemove(item, index) {
				...
				var url = this.common.baseNodeUrl+ "hrm/atd/Srv_HRM_AttendDeductDel.ashx";
				var ids = "['"+item.ID+"']";
			...
			cmdDetailLinkRemove(item, index) {
				...
				if(item.ID != undefined && item.ID != "") {
					var ids = "['"+item.ID+"']";
					var url = this.common.baseNodeUrl+"hrm/atd/Srv_HRM_AttendDeductDetailDel.ashx";
					this.$http.post(url,ids, {
							emulateJSON: true
						})
						.then(function(res) {
	 ...
	 // 导出
    cmdDetailExpExcel(){
        var url = this.common.baseNodeUrl+"hrm/atd/Srv_HRM_AttendDeductDetailExpExcel.ashx";
         this.$http.get(url)
					.then(function(res) {
						var dataJson = res.body;
						if(dataJson.Succeed != 1) {
		...
		//初始化
		created() {
			this.dateMonth.push(format(addMonths(Date.now(),-2), 'YYYYMM'));
			this.dateMonth.push(format(addMonths(Date.now(),-1), 'YYYYMM'));
		    this.dateMonth.push(format(Date.now(), 'YYYYMM'));
			this.dateMonth.push(format(addMonths(Date.now(),1), 'YYYYMM'));
			var userParam ="?pageName=modules/VUE/index.html@/index10";
			...
			var typeParam = this.common.comGetParam("PARENT_ID='HRM_LY1003'",typeObj);
