					...
					<div class="compile" v-show="isShowEdit">
							...
							<div class="input">
								...
								<div class="block"  style="display: inline-block;margin-left:0px;margin-bottom:10px ">
									...
									<select class="month" v-model="modelObj.FINANCE_CYCLE">
										<option :value="value.K" v-for="value in yearMonthList" >{{value.V}}</option>
									</select>
								</div>
								<p>
									<span>录入人：</span>
								...
								<p>
									...
									<input 
										type="text" 
										v-model="modelObj.COMMENTS" 
										style="height: 99px;" 
										:disabled="isDisabledInput"></p>
							</div>
						</div>
						<ul class="clearfix compile-bottom" 
							style="padding-left: 50px;padding-right: 20px;margin-top: 10px">
							<li class="fl">
								<el-button type="text" :disabled="btnActive.isDetailAdd" @click="cmdDetailAdd">
									<e class="add"></e>添加明细行</el-button>
							</li>
							<li class="fl">
								<el-button type="text" :disabled="btnActive.isDetailAdd" @click="cmdDetailImpData">
						...
						<div class="addmingxi">
							...
							<div 
								...
								v-bind:style="{ marginTop: topBox + 'px' }">
...
<script>
		...
		methods: {
			...
			saveIn() {
				...
				for(var k in this.modelObj.DetailList) {
					...
					// 	return false;
					// }
					this.modelObj.TOTAL = this.modelObj.TOTAL + this.modelObj.DetailList[k].MONEY;
			...
			cmdDetailImpData(){
				if(this.modelObj.ID !=undefined && this.modelObj.ID !=""){
					var url = this.common.baseNodeUrl+ "hrm/atd/Srv_HRM_AttendDeductDetailImpData.ashx?ID=" + this.modelObj.ID;
					this.$http.get(url)
						.then(function(res) {
						var dataJson = res.body;
						if(dataJson.Succeed != 1) {
						...
						this.cmdLinkEdit(this.modelObj,"ImpData");
					}).catch(function (res) {
	        			  console.log(res+"导入数据异常")
					});
				}else{
					alert("请先保存数据!");
				}
    ...
    cmdDetailExpExcel(){
		...
		created() {
			this.getMonths();
			var userParam ="?pageName=modules/VUE/index.html@/hrm_att_deduct";
