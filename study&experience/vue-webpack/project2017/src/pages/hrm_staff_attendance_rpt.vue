<template>
  ...
  <div class="info" style="width: 1061px">
    <div class="infoPlace">
      您所在的位置是：
      <span>人事管理 / 职工休假情况</span>
    </div>
    <div class="infoTitle clearfix">
      <div class="roster fl">
        <span>|</span>&nbsp;职工休假情况</div>
...
export default {
  ...
  methods: {
    ...
    cmdSearchList() {
      ...
      var url = this.common.baseNodeUrl+"hrm/rpt/Srv_HRM_StaffAttendanceRpt.ashx" + param  
      this.$http.get(url)					  		 
					.then(function(res) {
    ...
    // 导出
    cmdExpExcel(){
        var url = this.common.baseNodeUrl+"hrm/rpt/Srv_HRM_StaffAttendanceRptExcel.ashx?financeCycle="+this.currentYear+''+this.currentMonth;
         this.$http.get(url)             
					.then(function(res) {
						var dataJson = res.body;
						if(dataJson.Succeed != 1) {
